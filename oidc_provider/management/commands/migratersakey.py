...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--ignore-media-root',
                            help='Migrate even if MEDIA_ROOT is unset', action='store_true')

    def handle(self, *args, **options):
        if settings.MEDIA_ROOT == "" and not options['ignore_media_root']:
            raise CommandError(
                "MEDIA_ROOT not set. You should set this to a writable directory before running"
                "this command, or bypass this warning with --ignore-media-root"
            )
        ...
        for key in RSAKeyDatabase.objects.all():
            ...
            if not options['copy']:
                key.delete()
            self.stdout.write("{} key with kid: {}".format(
                "Copied" if options['copy'] else "Moved", new.kid))
